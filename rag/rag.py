...
def _load_pdf(file_path: str) -> List[Document]:
    ...
    for chunk in pages:
        ...
        docs.append(Document(
            ...
            metadata={"source": str(file_path), "page": chunk.get('metadata', {}).get('page', 0)}
        ))
    return docs
...
def _extract_and_ocr_docx_images(doc, file_path: Path, outputimages: str) -> List[str]:
    ...
    for i, part in enumerate(image_parts, 1):
        ...
        if text.strip():
            ...
            ocr_parts.append(f"![图片 {i}]({img_path.name})\n[图片内容]: {text.strip()}")
...
def _sparse_search(query: str, k: int) -> List[Document]:
    ...
    print(f"分数列表：scores",scores)

    #sorted从小到大排序，这里取负从大到小并取k个 
    ranked = sorted(range(len(scores)), key=lambda i: -scores[i])[:k]

    print(f"取前k个切片对应的分数从大到小：",ranked)
    #根据切片分筛选   拿到命中的切片
    return [_chunk_docs[i] for i in ranked if scores[i] > 0 ]


def _rrf_fuse(dense_docs: List[Document], sparse_docs: List[Document],
              k: int = 60, top_n: int = TOP_K) -> List[Document]:
...
async def retrieve(query: str, k: int = TOP_K) -> List[Document]:
    ...
    #开启两个线程运行
    #因为dense_task，sparse_task 这两个在上面是放进协程等下面await 两个一起运行完
    #并发执行器
    #gather  如果传进去的是协程对象 则会调用create_task包装成协程任务  如果传进去的是任务 则直接返回该任务
    #只有gather  和 create_task会创建任务（将协程对象放入到事件循环中等待await执行）
...
#         model="gte-rerank-v2",
#         top_n=TOP_N,
#         api_key=os.getenv("QWEN_RERANK_API_KEY")
#     )
#     print(f"重排模型准备：",reranker)
#     return reranker
...
def reordering(query: str, docs: List[Document])->list[Document]:
    ...
    #按相关性从高到低排好序的、新的文档列表  compress_documents文档压缩器 并返回topn条文档
    try:
        reranker = LocalReranker.rerank(query,docs,TOP_N)
        # print(f"重排序com之后：",reranker)
