...
from rag.rag import KnowledgeBaseError, retrieve_sync, get_status,reordering
from langgraph.graph import StateGraph, START, END
from agent.langchina import agent
from rag.generatellm import generate_answer
...
class AgentState(TypedDict):
    question:str # 单问题
    session_id:str 
    sub_questions:Optional[list[str]]  #多问题 Optional 或的意思
    idx:Optional[int]  #问题序号
    intent_result:Optional[IntentResult]  #意图
    contexts:Optional[str] #检索得到的答案
    answer:Optional[str] #响应
    meta:Optional[dict]  #元数组数据

    #封装节点，因为是先写的代码后面加的langraph
...
def route_after_split(state:AgentState)->str:
    ...
    return "multi_loop" if len(state.get("sub_questions") or []) >1 else "question_flow"
...
def short_circuit_node(state: AgentState) -> dict:
    ...
    if intent == IntentName.STATUS_QUERY:         # 状态查询：实时查库拼话术
        ...
        ) if status.get("initialized") else "知识库尚未初始化，请先上传文档。"
        return {"answer": text}
    if intent == IntentName.AMBIGUOUS and result and result.confidence == 0:
        return {"answer": "您好，我没收到您的问题，请重新输入您想查询的内容。"}
    return {"answer": SHORT_CIRCUIT_REPLIES.get(intent, "抱歉，我暂时无法回答这个问题。")}
...
def rag_node(state: AgentState) -> dict:
    ...
    try:
        ...
        #生成
        result = generate_answer(state["question"],docs)

    except KnowledgeBaseError as e:
        print(f"知识库不可用: {e}")
        answer = str(e)                     # 连接失败/空库 → 把具体提示语原样返回给用户
    except Exception as e:
        print(f"知识库检索失败: {e}")
...
def build_question_graph():
    ...
    for n in ("rag_flow", "agent_flow", "short_circuit"):
        g.add_edge(n, "merge")
...
def build_main_graph():
    ...
    g.add_conditional_edges("splitter", route_after_split, {
